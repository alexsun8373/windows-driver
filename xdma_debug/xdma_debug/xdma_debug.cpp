// xdma_debug.cpp : Defines the entry point for the console application.
//
...
static Options options = { TRUE, NULL, NULL, NULL, 0, 0, C2H, C2H, FALSE };
...
static int get_devices(GUID guid, TCHAR * devpath, size_t len_devpath) {
...
int __cdecl test(int argc, char* argv[]);

#define CONVER_10_TO_16             "0x%x"
#define WRITE_BUFFER_SIZE			512
int __cdecl main(int argc, char* argv[]) {
	int argc_write = 1;
	int argc_read = 1;
	//xdma_rw.exe c2h_0 read 0x100 -l 0x1000
	char* argv_write[WRITE_BUFFER_SIZE + 4];
	//xdma_rw.exe c2h_0 read 0x100 -l 0x1000
	char* argv_read[6];
	char HEX_NUM[7];
	...
	//xdma_rw <device> <read|write> <address> [OPTIONS] [DATA]
	//device
	//argv_write[0] is used in the old xdma_rw code for input format error output,
	//here not used
	argc_write++;
	argv_write[1] = (char *)malloc(sizeof("h2c_0"));
	strcpy_s(argv_write[1], sizeof("h2c_0"), "h2c_0");

	//write operation
	argc_write++;
	argv_write[2] = (char *)malloc(sizeof("write"));
	strcpy_s(argv_write[2], sizeof("write"), "write");

	//start address is 0x0
	argc_write++;
	argv_write[3] = (char *)malloc(sizeof("0x0"));
	strcpy_s(argv_write[3], sizeof("0x0"), "0x0");

	//copy 0-4095 to the write data buffer
	for (int i = 0; i < (WRITE_BUFFER_SIZE + 4); i++)
	{
		argc_write++;
		sprintf_s(HEX_NUM, 7, CONVER_10_TO_16, i);
		argv_write[i + 4] = (char *)malloc(sizeof(HEX_NUM));
		strcpy_s(argv_write[i + 4], sizeof(HEX_NUM), HEX_NUM);
	}

	//process write operation
	test(argc_write, argv_write);

	verbose_msg("Write Done\n");
	/*=====================================Read=======================================*/
	argc_read++;
	argv_read[1] = (char *)malloc(sizeof("c2h_0"));
	strcpy_s(argv_read[1], sizeof("c2h_0"), "c2h_0");

	//write operation
	argc_read++;
	argv_read[2] = (char *)malloc(sizeof("read"));
	strcpy_s(argv_read[2], sizeof("read"), "read");

	//start address is 0x0
	argc_read++;
	argv_read[3] = (char *)malloc(sizeof("0x0"));
	strcpy_s(argv_read[3], sizeof("0x0"), "0x0");

	//read length command
	argc_read++;
	argv_read[4] = (char *)malloc(sizeof("-l"));
	strcpy_s(argv_read[4], sizeof("-l"), "-l");

	//read length
	argc_read++;
	argv_read[5] = (char *)malloc(sizeof("0x1000"));
	strcpy_s(argv_read[5], sizeof("0x1000"), "0x1000");

	verbose_msg("read start\n");
	//process read operation
	//test(argc_read, argv_read);

	//for (int i = 0; i < 4096; i++) {
	//	if (options.data[i] == argv_write[i + 4])
	//		fprintf("Write Value: %s ; Read Value %s ; Equal", argv_write[i + 4], options.data[i]);
	//}
	//
...
int __cdecl test(int argc, char* argv[]) {
	...
	DWORD num_devices = get_devices(GUID_DEVINTERFACE_XDMA, (TCHAR *)device_base_path, sizeof(device_base_path));
	...
	// open device file
	HANDLE device = CreateFileA(device_path, GENERIC_READ | GENERIC_WRITE, 0, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
